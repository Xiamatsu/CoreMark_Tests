...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
#define CCR_CLEAR_MASK   ((uint32_t)0x00000000) /* DMA Channel config registers Masks */
...
void DMA_DeInit(DMA_Channel_TypeDef* DMAy_Channelx)
{
    ...
    DMAy_Channelx->CHCR &= (uint16_t)(~DMA_Channel_CHCR_EN);

    /* Reset DMAy Channelx control register */
    DMAy_Channelx->CHCR  = 0;

    /* Reset DMAy Channelx remaining bytes register */
    DMAy_Channelx->CHDTR = 0;

    /* Reset DMAy Channelx peripheral address register */
    DMAy_Channelx->CHPAR  = 0;

    /* Reset DMAy Channelx memory address register */
    DMAy_Channelx->CHMAR = 0;

    if (DMAy_Channelx == DMA_Channel1)
    {
        /* Reset interrupt pending bits for DMA Channel1 */
        DMA->IFCR |= DMA1_CHANNEL1_IT_MASK;
    }
    else if (DMAy_Channelx == DMA_Channel2)
    {
        /* Reset interrupt pending bits for DMA Channel2 */
        DMA->IFCR |= DMA1_CHANNEL2_IT_MASK;
    }
    else if (DMAy_Channelx == DMA_Channel3)
    {
        /* Reset interrupt pending bits for DMA Channel3 */
        DMA->IFCR |= DMA1_CHANNEL3_IT_MASK;
    }
    else if (DMAy_Channelx == DMA_Channel4)
    {
        /* Reset interrupt pending bits for DMA Channel4 */
        DMA->IFCR |= DMA1_CHANNEL4_IT_MASK;
    }
    else if (DMAy_Channelx == DMA_Channel5)
    {
        /* Reset interrupt pending bits for DMA Channel5 */
        DMA->IFCR |= DMA1_CHANNEL5_IT_MASK;
    }
    else if (DMAy_Channelx == DMA_Channel6)
    {
        /* Reset interrupt pending bits for DMA Channel6 */
        DMA->IFCR |= DMA1_CHANNEL6_IT_MASK;
    }
    else
    {
        if (DMAy_Channelx == DMA_Channel7)
        {
            /* Reset interrupt pending bits for DMA Channel7 */
            DMA->IFCR |= DMA1_CHANNEL7_IT_MASK;
        }
    }
}
...
void DMA_Init(DMA_Channel_TypeDef* DMAy_Channelx, DMA_InitTypeDef* DMA_InitStruct)
{
    ...
    DMAy_Channelx->CHCR = tmpreg;

    /*--------------------------- DMAy Channelx CNDTR Configuration --------------*/
    /* Write to DMAy Channelx CNDTR */
    DMAy_Channelx->CHDTR = DMA_InitStruct->DMA_BufferSize;

    /*--------------------------- DMAy Channelx CPAR Configuration ---------------*/
    /* Write to DMAy Channelx CPAR */
    DMAy_Channelx->CHPAR = DMA_InitStruct->DMA_PeripheralBaseAddr;

    /*--------------------------- DMAy Channelx CMAR Configuration ---------------*/
    /* Write to DMAy Channelx CMAR */
    DMAy_Channelx->CHMAR = DMA_InitStruct->DMA_MemoryBaseAddr;
}
...
void DMA_Cmd(DMA_Channel_TypeDef* DMAy_Channelx, FunctionalState NewState)
{
    ...
    if (NewState != DISABLE)
    {
        ...
        DMAy_Channelx->CHCR |= DMA_Channel_CHCR_EN;
    }
    else
    {
        /* Disable the selected DMAy Channelx */
...
/**
  ...
  * @param  DMAy: where y use to select the DMA peripheral.
  * @param  DMAy_CHx_RemapRequest: where y use to select the DMA and x can be 1 to 7
  *         for DMA1 to select the DMA1 Channel.
  * @retval None
  */
void DMA_RemapConfig(DMA_TypeDef* DMAy, uint32_t DMAy_CHx_RemapRequest)
{
    assert_param(IS_DMA_ALL_LIST(DMAy));
    assert_param(IS_DMA1_REMAP(DMAy_CHx_RemapRequest));

    DMAy->CHSELR &= ~((uint32_t)0x0F << (uint32_t)((DMAy_CHx_RemapRequest >> 28) * 4));
    DMAy->CHSELR |= (uint32_t)(DMAy_CHx_RemapRequest & 0x0FFFFFFF);
}
...
void DMA_SetCurrDataCounter(DMA_Channel_TypeDef* DMAy_Channelx, uint16_t DataNumber)
{
    ...
    /*--------------------------- DMAy Channelx CNDTR Configuration --------------*/
    /* Write to DMAy Channelx CNDTR */
    DMAy_Channelx->CHDTR = DataNumber;
}
...
uint16_t DMA_GetCurrDataCounter(DMA_Channel_TypeDef* DMAy_Channelx)
{
    ...
    return ((uint16_t)(DMAy_Channelx->CHDTR));
}
